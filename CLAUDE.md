...
3. **村はユーザーが始める前からあった。** 増える村人は「よそから来た人」ではなく「もとからいて、まだ語られていなかった人」。UI文言もプロンプトもこの枠で書く（「迎えにいく」ではなく「訪ねる」、「村に来た日」ではなく「あなたが知った日」）。
4. **一度与えた設定は変更不可。** ユーザーもAIも村人の設定を書き換えられない（実装では `Object.freeze`）。※例外として「立ち絵の画像」だけはユーザーが差し替え／削除できる。
5. **設定はパラメータの数値ではなく、エピソードを含む散文で提示する。**
...
- `buildInitialPrompt()`（村＋最初の3人）／`buildNewVillagerPrompt()`（増員）。出力は有効なJSONのみ。増員では `makeBonds()` が既存村人から1〜3人と縁の種（`BOND_KINDS`）を抽選し、その人たちの設定を全文渡す。
...
- **村人増員**: `WEEK = 7日`ごとに1人、`MAX_AUTO_POP = 25` まで自動。到達後は「新しい村人を招く」ボタン。増員のときだけ `makeConstraints(既存の年齢)` を渡し、`balancedAge` が足りていない年齢の段を厚く引く（村の年齢構成が `AGE_PYRAMID` の割合へ寄る。最初の3人は補正なし）。
...
- **`portraitSVG` の `rnd()` の呼び出し順**: 服の色・かたむき・眉2つ・口もと・前髪・目の型の順で、名前と年齢から再現している（保存はしていない）。手前に呼び出しを足したり順番を変えたりすると、**すでにいる村人の顔つきまで変わる**。増やすときは末尾に足す。`CLOTH_PALETTE` / `FRINGES` / `EYE_SHAPES` の要素数を変えたときも同じ。
